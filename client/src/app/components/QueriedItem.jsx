...
class QueriedItem extends React.Component {
    ...
    render() {
        let { item } = this.props;
        let weight =
            item.relationshipWeight > 1
                ? 100
                : parseInt(item.relationshipWeight * 100);
        ...
        switch (endNode.type) {
            case "file":
                ...
                title =
                    "../" + addr.slice(Math.max(addr.length - 3, 1)).join("/");
                break;
            case "url":
                title = endNode.title;
                if (title.length === 0) {
        ...
        if (item.relationshipWeight / item.avgOpen > 1) {
            ...
            if (item.relationshipWeight > 0) {
                ...
                raw = item.relationshipWeight.toFixed(3);
                weightValue = (Math.log10(
                    item.relationshipWeight / maxValue * 100
                ) * 10).toFixed(3);
                ...
                <IconText icon={iconClass} iconColor={typeIconColor}>
                    ...
                    >
                        ...
                        <div style={{ marginRight: 4, marginLeft: 4 }}>
                            <ThumbsDownIcon
                                onClick={() =>
                                    this._sendFeedback("negative", item)}
                            />
                        </div>
                        <div style={{ marginRight: 4, marginLeft: 4 }}>
                            <ThumbsUpIcon
                                onClick={() =>
                                    this._sendFeedback("positive", item)}
                            />
                        </div>
                    </div>
                </IconText>
