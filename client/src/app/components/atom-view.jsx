...
class AtomView extends React.Component {
    ...
    async handleEventTickerItemClick(nodeId) {
        let result = await agent.post("http://localhost:3000/query", {
            nodeId: nodeId
        });
        setTimeout(() => {
            this.setState(
    ...
    isLoggedIn() {
        ...
        console.log(
            "HEY",
            !_.isUndefined(localStorage.userId) &&
                !_.isNull(localStorage.userId)
        );
        return (
            !_.isUndefined(localStorage.userId) &&
            !_.isNull(localStorage.userId) &&
    ...
    render() {
                ...
                <Card style={LOCAL_STYLES.focusedItem} zDepth={5}>
                    <div>
                        <IconText icon={iconClass} iconColor={iconColor}>
                            {(focusedNode.title || focusedNode.address)
                                .split("/")
                                .filter(item => item !== "")
                                .slice(-1)
                                .pop()}
        ...
        let users = [
            { username: "parties", id: 83408 },
            { username: "roieki", id: 83258 }
        ];
        let body;
            ...
            <div>
                ...
                <div>
                    ...
                    <UserList
                        ...
                        onClick={this.handleUserFilter.bind(this)}
                    />
                    <hr />
                    <FlipMove>
                        {queriedItems}
