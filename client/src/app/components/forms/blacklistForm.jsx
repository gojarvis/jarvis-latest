...
class BlackListForm extends Component {
    ...
    async toggleBlackList() {
        ...
        let res = await agent.post("/api/user/setFilterStatus", {
    ...
    async getBlackListStatus() {
        let res = await agent.post("/api/user/getFilterStatus", {
            filterType: "blacklist"
        });
    ...
    async saveBlacklistExpression(expression) {
        ...
        let relationshipBetweenUserExpression = await agent.post(
            "/api/user/saveFilterExpression",
    ...
    async getBlacklistExpressions() {
        let res = await agent.post("/api/user/listFilterExpressions", {
            expressionType: "blacklist"
        });
    ...
    async deleteBlacklistExpression(expression) {
        let expressionType = "blacklist";
        let deleted = await agent.post("/api/user/deleteFilterExpression", {
            expression,
            expressionType
        });
        this.getBlacklistExpressions();
    ...
    render() {
        let blacklistEnabledLabel = this.state.blacklistEnabled
            ? "Black list enabled"
            : "Black list disabled";
        let blacklistToggle = (
            <Toggle
        ...
        return (
            <div>
                <ListItem
                    primaryText={blacklistEnabledLabel}
                    rightToggle={blacklistToggle}
                />
                <UrlFilteringEditor
