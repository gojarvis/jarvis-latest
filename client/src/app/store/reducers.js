...
import {
  ...
  SET_END_NODE_TYPE, TOGGLE_FILTER_BY_USER_ID,
  REQUEST_QUERY_ITEMS, RECEIVE_QUERY_ITEMS,
  REQUEST_BLACKLIST_NODE, RECEIVE_BLACKLIST_NODE,
  REQUEST_USER_JSON, REQUEST_TEAM_JSON,
  RECEIVE_USER_JSON, RECEIVE_TEAM_JSON,
} from './actionCreators';
...
function queriedItems(state = {
  ...
  switch (action.type) {
    ...
    case TOGGLE_FILTER_BY_USER_ID:
      return {
        ...state,
        endUserNodeIds: state.endUserNodeIds.includes(action.payload) ? state.endUserNodeIds.delete(action.payload) : state.endUserNodeIds.add(action.payload)
      };
