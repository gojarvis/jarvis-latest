...
const addTodo = async (driver, key) => {
    // add todo
    driver
        .findElement(webdriver.By.className("new-todo"))
...
describe("window (popup) page", function() {
    ...
    it("should can add todo", async () => {
        ...
        const text = await todo
            .findElement(webdriver.By.tagName("label"))
            .getText();
    ...
    it("should can complete all todos", async () => {
        ...
        const classNames = await Promise.all(
            todos.map(todo => todo.getAttribute("class"))
        );
    ...
    it("should can clear completed todos if completed todos count > 0", async () => {
        ...
        this.driver
            .findElement(webdriver.By.className("clear-completed"))
            .click();
        const todos = await findList(this.driver);
        const classNames = await Promise.all(
            todos.map(todo => todo.getAttribute("class"))
    ...
    it("should cannot clear completed todos if completed todos count = 0", async () => {
        const todos = await this.driver.findElements(
            webdriver.By.className("clear-completed")
        );
    ...
    it("should can filter active todos", async () => {
        ...
        await completeTodo(this.driver, 0);
        let todos = await this.driver.findElements(
            webdriver.By.css(".filters > li")
        );
        todos[1].click();
        await delay(1000);
