...
let alchemyLanguage = watson.alchemy_language({api_key: 'ab2b4727617c0d529641168272d1e661634feb72'});
let projectSettingsManager = require('../utils/settings-manager');


let alchemyBlacklist = [
  '.*google\.com.*',
  '.*localhost.*',
  '.*facebook\.com*'
];

let alchemyWhitelist = [
  'https?://.*',
]
...
class KeywordsManager{
  ...
  async fetchKeywordsForUrlFromAlchemy(urlNode){
    ...
    if (urlNode.alchemy === true){
      ...
      relationships = await graphUtil.updateNodeAndRelationships(urlNode, keywordsNodes, keywords);
      ...
      return new Promise(function(resolve, reject) {
        try {
          ...
          alchemyLanguage.keywords(params, function (err, response) {
            if (err) {
              console.log('error fetching keywords from alchemy:', err);
              reject(err);
            } else {
