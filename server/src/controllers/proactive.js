...
class Proactive {
    ...
    async relateContextToItself() {
        try {
            ...
            // console.log(urls);
            if (urls.length > 0) {
                let urlRelationships = Promise.all(
                    urls.map(url => this.relateUrlToUrls(url, urls))
                );

                let keywords = await Promise.all(
                    urls.map(url => this.metadata.getSetKeywordsForUrl(url))
                );
            }

            if (files.length > 0) {
                let fileRelationships = Promise.all(
                    files.map(file =>
            ...
            if (files.length > 0 && urls.length > 0) {
                let fileRelationships = Promise.all(
                    files.map(file =>
                        this.graph.relateOneToMany(file, urls, "openwith")
                    )
                );
            }
        } catch (err) {
    ...
    async relateUrlToUrls(url, urls) {
        ...
        let singleRelationships = await this.graph.relateOneToMany(
            url,
            others,
            "openwith"
        );
