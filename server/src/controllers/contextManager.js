...
class contextManager {
    ...
    async initContext(userInfo) {
        ...
        try {
            ...
            this.heart.createEvent(
                30,
                function(heartbeat, last) {
                    this.handleHeartbeat(heartbeat);
                }.bind(this)
            );

            this.slowHeart.createEvent(
                120,
                function(heartbeat, last) {
    ...
    async handleSlowHeartbeat(heartbeat) {
        ...
        let allReports = await ReportsController.getAllReports(
            context,
            user,
            modifiers
        );
    ...
    async getContextNodesBucketedByHour() {
        ...
        try {
            nodes = await Promise.all(
                nodeIdsByHours.map(item => this.getBucketedNodeById(item))
            );
    ...
    async getContextNodeIdsBucktedByHour() {
        ...
        try {
            numberOfHoursToAggregate =
                (await settingsManager.getAggregationHoursValue()) || 1;
            contextBucktededByHour = await r
                .table("Event")
                .filter(function(event) {
                    var eventDayHour = r.add(
                        event("timestamp").dayOfYear().mul(24),
                        event("timestamp").hours()
                    );
                    var nowDayHour = r.add(
                        r.now().dayOfYear().mul(24),
                        r.now().hours()
                    );
                    ...
                    return event
                        ...
                        .and(
                            eventDayHour.ge(
                                nowDayHour.sub(numberOfHoursToAggregate)
                            )
                        );
                })
                .map(function(row) {
                    return {
                        ...
                        dayHour: r.add(
                            row("timestamp").dayOfYear().mul(24),
                            row("timestamp").hours()
                        ),
    ...
    addCommandNode(commandNode) {
        let command = this.commands.filter(
            command => command.address === commandNode.address
        );
        if (command.length === 0) {
    ...
    async updateUserActivity() {
        ...
        try {
            ...
            let kw = await keywordsManager.fetchKeywordsForUrlFromAlchemy(
                urlNode
            );
        } catch (e) {
    ...
    async relateUrlsToFiles(urls, files) {
        let urlToFiles = await Promise.all(
            urls.map(url => graphUtil.relateOneToMany(url, files, "openwith"))
        );
        let filesToUrls = await Promise.all(
            files.map(file => graphUtil.relateOneToMany(file, urls, "openwith"))
        );
    }

    async relateUrlToUrls(url, urls) {
        let others = urls.filter(item => item.address !== url.address);
        let queries = await graphUtil.getRelateOneToManyQueries(
            url,
            others,
            "openwith"
        );
    ...
    async relateFileToFiles(file, files) {
        let others = files.filter(
            filterFile => file.address !== filterFile.address
        );
        let queries = await graphUtil.getRelateOneToManyQueries(
            file,
            others,
    ...
    async relateUserToContext() {
        ...
        if (!_.isEmpty(this.urls)) {
            let userToUrls = await graphUtil.relateOneToMany(
                this.user,
                this.urls,
                "touched"
            );
        ...
        if (!_.isEmpty(this.files)) {
            ...
            let userToFiles = await graphUtil.relateOneToMany(
                this.user,
                this.files,
                "touched"
            );
        ...
        if (!_.isEmpty(this.commands)) {
            let userToCommands = await graphUtil.relateOneToMany(
                this.user,
                this.commands,
                "touched"
            );
    ...
    async saveContext() {
        ...
        if (duration.asMinutes() > 2) {
            console.log(
                "Not active, time since last activity:",
                duration.asMinutes()
            );
            this.clearContext();
            return;
        ...
        try {
            ...
            let urls = await Promise.all(
                urlsArtifacts.map(urlsArtifact =>
                    graphUtil.saveUrl(urlsArtifact.url, urlsArtifact.title)
                )
            );
            this.urls = urls;
    ...
    async relateContextToItself() {
        ...
        try {
            ...
            if (urls.length > 0) {
                let urlRelationshipsQueries = await Promise.all(
                    urls.map(url => this.relateUrlToUrls(url, urls))
                );
                queries = queries.concat(urlRelationshipsQueries);
            }

            if (files.length > 0) {
                let fileRelationshipsQueries = await Promise.all(
                    files.map(file =>
                        this.relateFileToFiles(file, files, "openwith")
                    )
                );
            ...
            if (commands.length > 0) {
                let commandInternalRelationshipsQueries = await Promise.all(
                    commands.map(command =>
                        this.relateCommandToCommands(
                            command,
                            commands,
                            "openwith"
                        )
            ...
            if (files.length > 0 && urls.length > 0) {
                let filesToUrlsRelationshipsQueries = await Promise.all(
                    files.map(file =>
                        graphUtil.getRelateOneToManyQueries(
                            file,
                            urls,
                            "openwith"
                        )
                ...
                let urlsToFilesRelationshipsQueries = await Promise.all(
                    urls.map(url =>
                        graphUtil.getRelateOneToManyQueries(
                            url,
                            files,
            ...
            if (commands.length > 0 && files.length > 0) {
                let commandToFilesRelationshipsQueries = await Promise.all(
                    commands.map(command =>
                        graphUtil.getRelateOneToManyQueries(
                            command,
                            files,
                ...
                let filesToCommandsRelationshipsQueries = await Promise.all(
                    files.map(file =>
                        graphUtil.getRelateOneToManyQueries(
                            file,
                            commands,
            ...
            if (commands.length > 0 && urls.length > 0) {
                let commandToUrlsRelationshipsQueries = await Promise.all(
                    commands.map(command =>
                        graphUtil.getRelateOneToManyQueries(
                            command,
                            urls,
                ...
                let urlsToCommandsRelationshipsQueries = await Promise.all(
                    urls.map(url =>
                        graphUtil.getRelateOneToManyQueries(
                            url,
                            commands,
