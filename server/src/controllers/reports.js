...
class ReportsController {
    ...
    async getHotItemsInRepoBySource(source, threshold = 3) {
        ...
        try {
            nodes = await Promise.all(
                hotItemsIDsInRepo.map(item => this.getBucketedNodeById(item))
            );
    ...
    async getHotFilesInRepo(modifiers) {
        ...
        let hotFilesInRepo;
        try {
            hotFilesInRepo = await this.getHotItemsInRepoBySource(
                "atom",
                threshold
            );
        } catch (e) {
    ...
    async getHotURLsInRepo(modifiers) {
        ...
        try {
            hotUrlsInRepo = await this.getHotItemsInRepoBySource(
                "chrome",
                threshold
            );
        } catch (e) {
    ...
    async getSuggestedTeamMembers(context, user, modifiers) {
        ...
        if (!_.isEmpty(context)) {
            ...
            cypher = `match (user:User)-[r]-(n)-[o:openwith]-(m)-[t:touched]-(targetUser:User)
    ...
    async getAllReports(context, user, modifiers) {
        ...
        try {
            ...
            suggestedTeamMembers = await this.getSuggestedTeamMembers(
                context,
                user,
                modifiers
            );
            resourcesByOtherUsers = await this.getResourcesByOtherUsers(
                context,
                user,
                modifiers
            );
