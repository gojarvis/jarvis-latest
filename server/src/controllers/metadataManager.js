...
class MetadataManager {
    ...
    async getSetKeywordsForUrl(urlNode) {
        ...
        try {
            if (
                (_.isUndefined(urlNode.alchemy) ||
                    (!_.isUndefined(urlNode.alchemy) &&
                        urlNode.alchemy === "failed")) &&
                _.isUndefined(this.localCache[urlNode.address])
            ) {
                try {
                    ...
                    let results = await this.updateNodeAndRelationships(
                        urlNode,
                        keywordNodes
                    );
                    ...
                    let updatedNode = await this.updateUrlKeywordFetchStatus(
                        url,
                        "success"
                    );
            ...
            let relatedKeywords = await graphUtil.getRelatedToUrl(
                url,
                "related",
                1
            );
    ...
    async saveAndRelateKeywordsToUrl(keywords, urlNode) {
        ...
        keywordNodes.forEach(item => {
            let cypher = `START a=node(${item.node
                .id}), b=node(${updatedUrlNode.id}) MERGE a-[r:related]-b SET r.weight = coalesce(r.weight, 0) + ${item.relevance}`;
    ...
    async saveKeywords(keywords) {
        ...
        keywords.forEach(item => {
            let cypher = `MERGE (n:Keyword { type: 'keyword', text: '${item.get(
                "text"
            )}', alchemy: true}) RETURN n`;
            txn.query(cypher, {});
        });
    ...
    async relateKeywordToUrl(keyword, urlNode) {
        try {
            ...
            let relationship = await self.relateNodes(
                keywordNode,
                urlNode,
                "related",
                keyword.relevance
    ...
    async relateNodes(origin, target, relationship, relevance) {
        ...
        let cypher =
            ...
            relationship +
            "]-b " +
            "SET r.weight = coalesce(r.weight, 0) + " +
            rel;
        let params = {
            origin: origin.id,
            target: target.id,
