...
graph.constraints.uniqueness.create("User", "username", function(
    err,
    constraint
) {});
graph.constraints.uniqueness.create("Url", "address", function(
    err,
    constraint
) {});
graph.constraints.uniqueness.create("File", "address", function(
    err,
    constraint
) {});
graph.constraints.uniqueness.create("Command", "address", function(
    err,
    constraint
) {});
graph.constraints.uniqueness.create("Regex", "address", function(
    err,
    constraint
) {});
...
class GraphUtil {
    ...
    async getRelatedToUrl(url, relationship, threshold) {
        ...
        let cypher =
            ...
            '" AND r.weight > ' +
            threshold +
            "  RETURN r,q ORDER BY r.weight DESC LIMIT 10";
    ...
    async getRelevantUrls() {
        ...
        let related = await Promise.all(
            urls.map(url => graph.getRelatedToUrl(url, "openwith", 30))
        );
        // console.log(related);
        return related;
    }
    ...
    async relateOneToMany(origin, others, relationship) {
        ...
        try {
            ...
            let relationshipQueries = others.map(target =>
                this.getRelateNodeQuery(origin, target, relationship)
            );
    ...
    async relateManyToOne(many, origin, relationship) {
        ...
        try {
            ...
            let relationshipQueries = many.map(target =>
                this.getRelateNodeQuery(target, origin, relationship)
            );
            let results = await this.executeQueries(relationshipQueries);
        } catch (e) {
    ...
    async relateNodes(origin, target, relationship) {
        ...
        let cypher =
            "START a=node({origin}), b=node({target}) " +
            "MERGE (a)-[r:" +
            relationship +
            "]->(b) " +
    ...
    async getRelatedNodes(startNode, relationship) {
        let cypher = `MATCH (startNode)-[relationship:${relationship}]->(endNode) where ID(startNode)=${startNode.id} return endNode`;
        let res = {};
        try {
            res = await this.queryGraph(cypher);
        } catch (err) {
            console.log(
                "failed to get related nodes in graphUtil",
                err,
                cypher
            );
        } finally {
    ...
    async getSaveUserInGraph(user) {
        ...
        try {
            ...
            if (_.isEmpty(userNode)) {
                ...
                userDefaultFilters = await this.saveUserDefaultFilters(
                    userNode
                );
            }
        } catch (e) {
    ...
    async saveUserDefaultFilters(userNode) {
        ...
        let blacklistExpressionsNodes = await Promise.all(
            blacklistExpressions.map(expression => this.saveRegex(expression))
        );
        let whitelistExpressionsNodes = await Promise.all(
            whitelistExpressions.map(expression => this.saveRegex(expression))
        );

        let blacklistRelationships = await Promise.all(
            blacklistExpressionsNodes.map(expressionNode =>
                this.relateNodes(userNode, expressionNode, "blacklist")
            )
        );
        let whitelistRelationships = await Promise.all(
            whitelistExpressionsNodes.map(expressionNode =>
                this.relateNodes(userNode, expressionNode, "whitelist")
            )
        );
    ...
    saveFile(address) {
        ...
        return new Promise(function(resolve, reject) {
            graph.save(
                { type: "file", address: trimmedAddress },
                "File",
                function(err, node) {
    ...
    getRegex(address) {
        let self = this;
        return new Promise(function(resolve, reject) {
            graph.find({ type: "regex", address: address }, function(
                err,
                node
            ) {
    ...
    getCommand(command) {
        let self = this;
        return new Promise(function(resolve, reject) {
            graph.find({ type: "command", address: command }, function(
                err,
                node
            ) {
                node = node ? node[0] : false;
    ...
    async updateNodeAndRelationships(urlNode, keywordNodes, keywords) {
        ...
        MERGE (a)-[r:related]->(b)
        SET r.weight = coalesce(r.weight, 0) + ${keywordTextToRelevanceHash[
            kwObj.text
        ]}`;
